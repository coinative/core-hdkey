...
function HDKey(data) {
  ...
  if (data.prv) {
    this.prv = data.prv;
    this._key = new Key({ private: data.prv });
  } else {
    this._key = new Key({ public: data.pub, compressed: true });
  }
  this.pub = this._key.public;
...
Object.defineProperty(HDKey.prototype, 'address', {
  get: function () {
...
HDKey.prototype.derive = function (index) {
  ...
  if (this.prv) {
    ...
    child.pub = derive.derivePublic(IL, this.pub);
  }
